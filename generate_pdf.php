...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, 'Details de la Collecte', 0, 1, 'C');
        $this->Ln(10);
    }
...
function generatePDF($collection_id, $conn, $path)
{
    ...
    $pdf->Cell(0, 10, 'Bénévole: ' . sanitizeString($result['volunteer_name'] ?: 'Non attribue'), 0, 1);

    // Détails de la livraison
    $pdf->Cell(0, 10, 'Date de Livraison: ' . sanitizeString($result['delivery_date'] ?: 'Non disponible'), 0, 1);
    $pdf->Cell(0, 10, 'Statut de la Livraison: ' . sanitizeString($result['delivery_status']), 0, 1);
    $pdf->Cell(0, 10, 'Beneficiaire: ' . sanitizeString($result['beneficiary_name'] ?: 'Non attribue'), 0, 1);
    $pdf->Cell(0, 10, 'Adresse du Beneficiaire: ' . sanitizeString($result['beneficiary_address'] ?: 'Non disponible'), 0, 1);
